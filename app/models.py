...
Using Pydantic v2 with proper field validation and defaults.
"""
...
class PrayerTimesResponse(BaseModel):
    ...
    @classmethod
    def from_cached_data(cls, cached_data: Dict, cache_hit: bool = True) -> "PrayerTimesResponse":
        """Create response from cached data."""
        return cls(
            date=cached_data.get("date"),
            location=Location(**cached_data.get("location", {})),
            country=cached_data.get("country"),
            timezone_offset=cached_data.get("timezone_offset"),
            prayer_times=cached_data.get("prayer_times", {}),
            qibla_direction=cached_data.get("qibla_direction"),
            calibration_applied=cached_data.get("calibration_applied", True),
            cache_hit=cache_hit,
            calculation_time_ms=0.1 if cache_hit else 50.0,
...
class BulkPrayerTimesRequest(BaseModel):
    """Request model for bulk prayer times (18 months)."""
    latitude: float = Field(..., ge=-90, le=90)
    longitude: float = Field(..., ge=-180, le=180)
...
class BulkPrayerTimesResponse(BaseModel):
    ...
    cache_hit: bool = Field(default=False)
    calculation_time_ms: float = Field(default=0.0)
    months_included: int = Field(default=18)
    date_range: str = Field(default="Past 6 months + Future 12 months")
